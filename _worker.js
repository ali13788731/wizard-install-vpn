...
export default {
	async fetch(request, env, ctx) {
        ...
        // =====================================================================
        // License Management, Admin Panel & Fallback Site
        // =====================================================================
        const adminPath = 'my-secret-admin-9988'; 
        const adminPassword = 'admin12345'; 
        const fallbackSite = 'https://www.google.com'; 

        const currentPath = 访问路径;
        const clientIP = 访问IP;

        if (env.KV && typeof env.KV.get === 'function') {
            if (currentPath === adminPath) {
                const providedToken = url.searchParams.get('token');
                if (providedToken !== adminPassword) return new Response('Not Found', { status: 404 });
				...
				if (request.method === 'POST') {
                    ...
                    // اکشن دریافت اطلاعات زنده بدون آپدیت کردن دیتابیس
                    if (formData.action === 'getData') {
                        return new Response(JSON.stringify({ success: true, data: licenseData }), { headers: { 'Content-Type': 'application/json' } });
                    }

                    if (formData.action === 'updateExp') {
                        licenseData.exp = new Date(formData.date + 'T' + formData.time + ':00+03:30').getTime();
                        // اعمال همزمان وضعیت تک‌کاربره که از سمت ربات ارسال می‌شود
                        if (formData.singleUser !== undefined) {
                            licenseData.singleUser = formData.singleUser;
                        }
                    }
                    if (formData.action === 'toggleUser') licenseData.singleUser = !licenseData.singleUser;
                    if (formData.action === 'resetIP') { licenseData.activeIP = null; licenseData.lastSeen = 0; }
                    if (formData.action === 'toggleKillSwitch') licenseData.killSwitch = !licenseData.killSwitch; 
                    
                    await env.KV.put('license_data', JSON.stringify(licenseData));
                    ...
                    return new Response(JSON.stringify({ 
                        success: true, 
                        data: licenseData,
                        subLink: generatedSubLink // ارسال لینک به ربات
                ...
                let statusHTML = '';
                if (licenseData.killSwitch) {
                    statusHTML = '<span class="status" style="color:#dc3545">🛑 مسدود موقت (Kill Switch فعال است)</span>';
                } else if (licenseData.exp === 0) {
                    statusHTML = '<span class="status" style="color:#6c757d">⚪ غیرفعال (تاریخ انقضا تنظیم نشده)</span>';
                } else if (timeNow > licenseData.exp) {
                    statusHTML = '<span class="status" style="color:#dc3545">❌ منقضی شده (دسترسی قطع شد)</span>';
                } else {
                    statusHTML = '<span class="status" style="color:#28a745">✅ فعال و در حال کار</span>';
                }

                const tehranStr = new Date().toLocaleString("en-US", {timeZone: "Asia/Tehran"});
                let targetDate = new Date(tehranStr); 
                if (licenseData.exp !== 0) {
                    targetDate = new Date(new Date(licenseData.exp).toLocaleString("en-US", {timeZone: "Asia/Tehran"}));
                }
                const defDateStr = targetDate.getFullYear() + '-' + String(targetDate.getMonth()+1).padStart(2, '0') + '-' + String(targetDate.getDate()).padStart(2, '0');
                const defTimeStr = String(targetDate.getHours()).padStart(2, '0') + ':' + String(targetDate.getMinutes()).padStart(2, '0');

                const serverTimeShamsi = new Date().toLocaleString('fa-IR', { timeZone: 'Asia/Tehran' });
                const expTimeShamsi = licenseData.exp === 0 ? 'تنظیم نشده (پیش‌فرض: همین لحظه)' : new Date(licenseData.exp).toLocaleString('fa-IR', { timeZone: 'Asia/Tehran' });
                const lastSeenShamsi = licenseData.lastSeen === 0 ? '-' : new Date(licenseData.lastSeen).toLocaleTimeString('fa-IR', { timeZone: 'Asia/Tehran' });

                const html = `<!DOCTYPE html><html dir="rtl" lang="fa"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>پنل مدیریت اشتراک</title><style>body{font-family:Tahoma, Arial, sans-serif;background:#f0f2f5;padding:20px;color:#333} .card{background:#fff;padding:20px;border-radius:10px;box-shadow:0 4px 8px rgba(0,0,0,0.1);max-width:400px;margin:auto; margin-bottom:40px;} button{background:#007bff;color:#fff;border:none;padding:10px 15px;border-radius:5px;cursor:pointer;margin-top:10px;width:100%; font-family:Tahoma; transition:0.3s;} button:hover{opacity:0.8} input{padding:10px; border:1px solid #ccc;border-radius:5px; font-family:Tahoma;} .input-group{display:flex; gap:10px; margin-top:10px;} .input-group input{flex:1; width:100%;} .status{font-weight:bold;} .server-time{text-align:center; font-size: 0.9em; color: #555; background: #e9ecef; padding: 10px; border-radius: 5px; margin-bottom: 15px;}</style></head><body><div class="card"><h2 style="text-align:center; margin-top: 0;">مدیریت کانفیگ</h2>
                <div class="server-time">ساعت و تاریخ زنده سرور:<br><b id="liveClock" dir="ltr" style="color:#0056b3; font-size:1.15em; display:inline-block; margin-top:5px;">${serverTimeShamsi}</b></div>
                <p><b>وضعیت کلی سرویس:</b><br> <span id="globalStatus">${statusHTML}</span></p>
                <button id="ksBtn" onclick="sendReq({action:'toggleKillSwitch'})" style="background:${licenseData.killSwitch?'#28a745':'#dc3545'}; font-size:1.05em; padding:12px;">${licenseData.killSwitch ? '▶️ فعال‌سازی مجدد سرویس' : '🛑 توقف فوری سرویس (Kill Switch)'}</button>
                <hr>
                <p><b>تاریخ انقضای تنظیم شده:</b> <br><span id="expText" dir="ltr" style="color:#0056b3; font-weight:bold;">${expTimeShamsi}</span></p>
                
                <p style="background:#e8f4f8; padding:10px; border-radius:5px; border-right:4px solid #007bff; line-height: 1.6;">
                    <b>زمان باقی‌مانده:</b> <br>
                    <span id="countdownText" style="color:#007bff; font-weight:bold; font-size:1.05em;">در حال محاسبه...</span>
                </p>

                <small style="color:#777;">در صورت تغییر ندادن، تاریخ و ساعت روز در نظر گرفته می‌شود.</small>
                <div class="input-group">
                    <input type="time" id="expTime" value="${defTimeStr}">
                    <input type="date" id="expDate" value="${defDateStr}">
                </div>
                <button onclick="sendReq({action:'updateExp', date: document.getElementById('expDate').value, time: document.getElementById('expTime').value})">ذخیره تاریخ و ساعت جدید</button>
                <hr>
                <p><b>حالت تک کاربره:</b> <span id="suStatus" class="status" style="color:${licenseData.singleUser?'#dc3545':'#28a745'}">${licenseData.singleUser ? 'فعال (قفل روی یک IP)' : 'غیرفعال (آزاد)'}</span></p>
                <button onclick="sendReq({action:'toggleUser'})" style="background:#6c757d;">تغییر حالت تک کاربره</button>
                
                <div id="ipSection" style="display: ${licenseData.singleUser ? 'block' : 'none'}; background:#fff9e6; padding:10px; border-radius:5px; margin-top:15px; border-right:4px solid #ffc107;">
                    <p style="margin-top:0;"><b>آی‌پی متصل فعلی:</b> <span id="activeIP" dir="ltr" style="font-weight:bold;">${licenseData.activeIP || 'هیچکس'}</span></p>
                    <p><b>زمان آخرین بازدید:</b> <span id="lastSeen" dir="ltr">${lastSeenShamsi}</span></p>
                    <button onclick="sendReq({action:'resetIP'})" style="background:#dc3545">قطع اتصال آی‌پی فعلی (Kick)</button>
                </div>
                ...
                <script>
                    let sTime = ${timeNow};
                    let expTime = ${licenseData.exp};
                    
                    // آپدیت ساعت و ثانیه شمار زنده
                    setInterval(() => {
                        sTime += 1000;
                        document.getElementById('liveClock').innerText = new Date(sTime).toLocaleString('fa-IR', { timeZone: 'Asia/Tehran' });
                        
                        let cdTextElem = document.getElementById('countdownText');
                        if (expTime === 0) {
                            cdTextElem.innerText = "نامحدود (تنظیم نشده)";
                            cdTextElem.style.color = "#6c757d";
                        } else {
                            let diff = expTime - sTime;
                            if (diff <= 0) {
                                cdTextElem.innerText = "پایان یافته (سرویس قطع است)";
                                cdTextElem.style.color = "#dc3545";
                            } else {
                                ...
                                let cdStr = "";
                                if (days > 0) cdStr += days + " روز و ";
                                if (hours > 0 || days > 0) cdStr += hours + " ساعت و ";
                                cdStr += mins + " دقیقه و " + secs + " ثانیه دیگر";
                                
                                cdTextElem.innerText = cdStr;
                                cdTextElem.style.color = "#28a745";
                            }
                        }
                    }, 1000);
                    
                    // دریافت زنده اطلاعات (AJAX) هر 3 ثانیه
                    async function fetchUpdate() {
                        try {
                            let res = await fetch('', {method:'POST', body:JSON.stringify({action:'getData'})});
                    ...
                    // اعمال دیتا روی صفحه بدون رفرش
                    function applyData(d) {
                        expTime = d.exp;
                        
                        let ksBtn = document.getElementById('ksBtn');
                        if(d.killSwitch) {
                            ksBtn.style.background = '#28a745';
                            ksBtn.innerText = '▶️ فعال‌سازی مجدد سرویس';
                        } else {
                            ksBtn.style.background = '#dc3545';
                            ksBtn.innerText = '🛑 توقف فوری سرویس (Kill Switch)';
                        }

                        let suStatus = document.getElementById('suStatus');
                        suStatus.innerText = d.singleUser ? 'فعال (قفل روی یک IP)' : 'غیرفعال (آزاد)';
                        suStatus.style.color = d.singleUser ? '#dc3545' : '#28a745';

                        document.getElementById('ipSection').style.display = d.singleUser ? 'block' : 'none';
                        document.getElementById('activeIP').innerText = d.activeIP || 'هیچکس';
                        document.getElementById('lastSeen').innerText = d.lastSeen === 0 ? '-' : new Date(d.lastSeen).toLocaleTimeString('fa-IR', { timeZone: 'Asia/Tehran' });
                        document.getElementById('expText').innerText = d.exp === 0 ? 'تنظیم نشده (پیش‌فرض: همین لحظه)' : new Date(d.exp).toLocaleString('fa-IR', { timeZone: 'Asia/Tehran' });
                        
                        let stHtml = '';
                        if (d.killSwitch) stHtml = '<span class="status" style="color:#dc3545">🛑 مسدود موقت (Kill Switch فعال است)</span>';
                        else if (d.exp === 0) stHtml = '<span class="status" style="color:#6c757d">⚪ غیرفعال (تاریخ انقضا تنظیم نشده)</span>';
                        else if (sTime > d.exp) stHtml = '<span class="status" style="color:#dc3545">❌ منقضی شده (دسترسی قطع شد)</span>';
                        else stHtml = '<span class="status" style="color:#28a745">✅ فعال و در حال کار</span>';
                        document.getElementById('globalStatus').innerHTML = stHtml;
                    }

                    // ارسال درخواست (بدون رفرش)
                    async function sendReq(data) {
                        try {
                            let res = await fetch('', {method:'POST', body:JSON.stringify(data)});
                            let json = await res.json();
                            if(json.data) applyData(json.data);
                        } catch(e) {
            ...
            if (licenseData.singleUser) {
                if (licenseData.activeIP && licenseData.activeIP !== clientIP) {
                    if ((currentTime - licenseData.lastSeen) < 30000) {
                        return new Response(null, { status: 444 }); 
                    }
                }
                if (licenseData.activeIP !== clientIP || (currentTime - licenseData.lastSeen) > 15000) {
                    licenseData.activeIP = clientIP;
                    licenseData.lastSeen = currentTime;
                    ctx.waitUntil(env.KV.put('license_data', JSON.stringify(licenseData)));
                }
            }
        }
        // =====================================================================
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						...
						if (订阅类型 === 'mixed') {
							...
							// 👇 کدهای زیر را دقیقاً در این قسمت قرار دهید 👇
							// --- [شروع: افزودن تاریخ شمسی به کانفیگ‌ها] ---
							const shamsiNow = new Date().toLocaleString('fa-IR', {
							...
							订阅内容 = 订阅内容.split('\n').map(line => {
								...
								if (line.toLowerCase().startsWith('vmess://')) {
									try {
										let jsonObj = JSON.parse(atob(line.slice(8).split('#')[0]));
										let ps = (jsonObj.ps || 'CF-Node')
											.replace(/CF移动优选/g, 'CF-MCI')
											.replace(/CF联通优选/g, 'CF-Irancell')
											.replace(/CF电信优选/g, 'CF-Telecom')
											.replace(/CF官方优选/g, 'CF-Official')
											.replace(/CF优选/g, 'CF-Opt')
											.replace(/优选/g, 'Opt')
											.replace(/[\u4e00-\u9fa5]/g, '')
											.replace(/\s*-\s*[0-9۰-۹]{4}\/[0-9۰-۹]{1,2}\/[0-9۰-۹]{1,2}.*$/, '')
											.trim();
										if (!ps) ps = 'CF-Node';
										jsonObj.ps = `${ps} - ${shamsiNow}`;
										return 'vmess://' + btoa(JSON.stringify(jsonObj));
									} catch(e) { return line; }
								} else if (line.includes('://')) {
									let [urlPart, ...remarkParts] = line.split('#');
									let remark = remarkParts.join('#');
									if (!remark) {
										try { remark = new URL(urlPart).hostname || 'CF-Node'; } 
										catch(e) { remark = 'CF-Node'; }
									} else {
										try { remark = decodeURIComponent(remark); } catch(e){}
									}
									...
									if (!remark) remark = 'CF-Node';
									remark = `${remark} - ${shamsiNow}`;
									return `${urlPart}#${encodeURIComponent(remark)}`;
								}
								return line;
							}).join('\n');
							// --- [پایان: افزودن تاریخ شمسی] ---
							// 👆 پایان کدهای جدید 👆
						} else { // 订阅转换
							// کدهای مربوط به تبدیل ساب‌اسکریپشن (ساب‌کانورتر) از اینجا به بعد قرار دارند...
							...
							// --- [افزودن تاریخ شمسی و حذف کاراکترهای چینی از نام کانفیگ‌ها] ---
							const shamsiNow = new Date().toLocaleString('fa-IR', {
								timeZone: 'Asia/Tehran',
								year: 'numeric',
								month: '2-digit',
								...
								hour: '2-digit',
								minute: '2-digit',
								second: '2-digit', // ثانیه در اینجا اضافه شد
								hour12: false
							}).replace(/,/g, '');
							...
							订阅内容 = 订阅内容.split('\n').map(line => {
								...
								if (line.toLowerCase().startsWith('vmess://')) {
									try {
										...
										let ps = (jsonObj.ps || 'CF-Node')
											...
											.replace(/优选/g, 'Opt')
											.replace(/[\u4e00-\u9fa5]/g, '')
											.replace(/\s*-\s*[0-9۰-۹]{4}\/[0-9۰-۹]{1,2}\/[0-9۰-۹]{1,2}.*$/, '') // حذف تاریخ قبلی در صورت وجود
											.trim();
										if (!ps) ps = 'CF-Node';
										jsonObj.ps = `${ps} - ${shamsiNow}`;
										return 'vmess://' + btoa(JSON.stringify(jsonObj));
								...
								// پشتیبانی از لینک‌های vless, trojan, ss و غیره
								let [urlPart, ...remarkParts] = line.split('#');
								let remark = remarkParts.join('#');
								if (!remark) {
									try { remark = new URL(urlPart).hostname || 'CF-Node'; } 
								...
								// جایگزینی نام‌های اپراتوری و حذف کامل کاراکترهای چینی
								remark = remark
									.replace(/CF移动优选/g, 'CF-MCI')
									.replace(/CF联通优选/g, 'CF-Irancell')
									.replace(/CF电信优选/g, 'CF-Telecom')
								...
								// افزودن خط فاصله و تاریخ شمسی (شامل ساعت، دقیقه و ثانیه)
								remark = `${remark} - ${shamsiNow}`;

								return `${urlPart}#${encodeURIComponent(remark)}`;
							}).join('\n');
							// --- [پایان ویرایش نام کانفیگ‌ها] ---					
...
async function 生成随机IP(request, count = 16, 指定端口 = -1) {
	...
	const 运营商名称映射 = {
		cmcc: 'CF-MCI',       // همراه اول / موبایل
		cu: 'CF-Irancell',    // ایرانسل / رایتل
		ct: 'CF-Telecom',     // مخابرات
		cf: 'CF-Official',    // کلودفلر رسمی
	};
			...
			if (!isCSV) {
				...
				if (headers.includes('IP地址') && headers.includes('端口') && headers.includes('数据中心')) {
					...
					dataLines.forEach(line => {
						const cols = line.split(',').map(c => c.trim());
						const wrappedIP = IPV6_PATTERN.test(cols[ipIdx]) ? `[${cols[ipIdx]}]` : cols[ipIdx];
						const ipItem = `${wrappedIP}:${port}#CF-Opt ${cols[delayIdx]}ms ${cols[speedIdx]}MB/s`;
						// 处理第一个数组 - 优选IP
						if (API备注名) {
							const 处理后IP = `${ipItem} [${API备注名}]`;
							results.add(处理后IP);
